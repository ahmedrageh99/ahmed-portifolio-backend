...
const userSchema = new mongoose.Schema({ name: String, email: { type: String, unique: true, required: true }, password: { type: String, required: true } });
const User = mongoose.model('User', userSchema);

const projectSchema = new mongoose.Schema({ userId: { type: mongoose.Schema.Types.ObjectId, ref: 'User' }, name: String, url: String }, { timestamps: true });
const Project = mongoose.model('Project', projectSchema);

const todoSchema = new mongoose.Schema({ userId: { type: mongoose.Schema.Types.ObjectId, ref: 'User', index: true }, text: { type: String, required: true, trim: true, maxlength: 280 }, done: { type: Boolean, default: false } }, { timestamps: true });
const Todo = mongoose.model('Todo', todoSchema);

function authMiddleware(req,res,next){
  const token = req.headers.authorization?.split(' ')[1];
  if(!token) return res.status(401).json({ message: 'Unauthorized' });
  try{ const decoded = jwt.verify(token, SECRET); req.user = { userId: decoded.userId || decoded.id, email: decoded.email }; next(); }
...
app.post('/api/contact', (req,res)=>{
  ...
  console.log('Contact:', name, email); res.json({ message: 'Contact received' });
});
...
app.post('/api/register', async (req,res)=>{
  ...
  const exists = await User.findOne({ email }); if (exists) return res.status(400).json({ message: 'User already exists' });
...
app.post('/request-password-reset', async (req,res)=>{
  ...
  const token = crypto.randomBytes(32).toString('hex'); const expires = Date.now()+3600000;
  ...
  try{ await transporter.sendMail({ from: process.env.EMAIL_USER, to: email, subject:'Password Reset', text:`Reset your password: ${resetLink}` }); res.send('Password reset email sent'); }
  catch(e){ console.error(e); res.status(500).send('Error sending email'); }
});
app.post('/reset-password', async (req,res)=>{
  const { token, newPassword } = req.body || {};
  const record = passwordResetTokens.get(token);
  if(!record) return res.status(400).send('Invalid or expired token');
  if(Date.now()>record.expires){ passwordResetTokens.delete(token); return res.status(400).send('Token expired'); }
  const user = await User.findById(record.userId); if(!user) return res.status(404).send('User not found');
  user.password = await bcrypt.hash(newPassword, 10); await user.save();
  passwordResetTokens.delete(token); res.send('Password reset successfully');
});
